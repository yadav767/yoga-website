...
const AdminExperience = () => {
  ...
  const submitHandler = async (values) => {
    try {
      ...
      if (selectedItemForEdit) {
        response = await axios.post("https://yoga-hq9u.onrender.com/api/yoga/update-experience", { ...values, _id: selectedItemForEdit._id })
      } else {
        response = await axios.post("https://yoga-hq9u.onrender.com/api/yoga/add-experience", values)
      }
  ...
  const onDelete = async (item) => {
    try {
      dispatch(setLoading())
      const response = await axios.post("https://yoga-hq9u.onrender.com/api/yoga/delete-experience", { _id: item._id })
      dispatch(hideLoading())
      if (response.data.success) {
        message.success(response.data.message)
        dispatch(hideLoading())
      ...
      <div className="grid grid-cols-3 max-sm:grid-cols-1 gap-5">
        {experiences.map((experience, idx) => (
          <div key={idx} className='shadow border flex flex-col justify-between gap-4 p-5 border-gray-700'>
            <h1 className=' text-primary font-semibold text-xl'>{experience.title}</h1>
